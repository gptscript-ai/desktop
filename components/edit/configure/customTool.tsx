...
const CustomTool: React.FC<ConfigureProps> = ({ tool }) => {
  ...
  useEffect(() => {
    ...
    setTools((prevTools) => {
      const updatedTools = prevTools.map((t: Tool) => {
        // replace all instances of the old name with the new name
        t.tools?.map((tImport) =>
          tImport === customTool.name ? name : tImport
        );
  ...
  const setCustomToolTools = useCallback(
    (newTools: string[]) => {
  ...
  const setParams = useCallback(
    (newParams: Record<string, Property>) => {
      setCustomTool((prevCustomTool) => ({
        ...prevCustomTool,
        arguments: { properties: newParams, type: 'object' },
      }));
    },
    [setCustomTool]
  );
  ...
  const handleDelete = () => {
    setRoot((prevRoot) => {
      if (!prevRoot.tools) return prevRoot;
      prevRoot.tools = prevRoot.tools.filter(
        (tImport) => tImport !== customTool.name
      );
      return prevRoot;
    });

    setTools((prevTools) => {
      let updatedTools = prevTools.filter(
        (t: Tool) => t.name !== customTool.name
      );
              ...
              <div className="border-r-2 dark:border-zinc-700 pr-6 h-full overflow-y-auto">
                <div className="w-full flex flex-col justify-center space-y-4 mb-6">
                  ...
                  >
                    <Switch
                      ...
                      isSelected={chat}
                      onChange={(e) =>
                        setCustomTool({ ...customTool, chat: e.target.checked })
                      }
                ...
                <div className="px-2 flex flex-col space-y-4 mb-6">
                  ...
                  <Select
                    ...
                    variant="bordered"
                    defaultSelectedKeys={[
                      customTool.instructions?.startsWith('#!')
                        ? 'code'
                        : 'prompt',
                    ]}
                  ...
                  >
                    ...
                    >
                      ...
                      <p className="text-default-500 text-tiny">
                        Standard - Describe behavior using natural language.
                      </p>
                    </SelectItem>
                    <SelectItem
                      key="code"
                    ...
                    >
                      <h1>Code</h1>
                      <p className="text-default-500 text-tiny">
                        Advanced - Describe behavior using proramming languages.
                      </p>
                    </SelectItem>
                  </Select>
                  {instructionsType !== 'code' && (
                    <>
                      ...
                      <Accordion isCompact fullWidth selectionMode="multiple">
                        ...
                        >
                          <Params
                            params={customTool.arguments?.properties}
                            setParams={setParams}
                          />
                        </AccordionItem>
                        <AccordionItem
                        ...
                        <AccordionItem
                          ...
                          startContent={<HiCog />}
                          classNames={{
                            content: 'px-10 pt-6 h-[500px] space-y-4',
                          }}
                        >
                          <Models
                            options={models}
                  ...
                  {instructionsType === 'code' && (
                    <>
                      <Code
                        ...
                        onChange={(e) =>
                          setCustomTool({ ...customTool, instructions: e })
                        }
                        dependencies={
                          dependencies.find(
                            (d) => d.forTool === customTool.name
                          )?.content || ''
                        }
