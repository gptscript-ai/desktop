    ...
    "Working with Local Files": [
        {
            ...
            url: "github.com/gptscript-ai/knowledge@v0.4.5-gateway",
            tags: ["knowledge", "rag"],
...
const ToolCatalog: React.FC<ToolCatalogProps> = ({tools, addTool, removeTool}) => {
    ...
    const [priorityTools, setPriorityTools] = useState(featuredTools);
    ...
    return (
        <div>
            ...
            {searchResults && Object.keys(searchResults).length > 0 ? (
                <div>
                    { searchResults.tools && Object.keys(searchResults.tools).length === 0 && 
                        (
                            <div className="pl-12">
                                <p className="text-lg">No tools found. Please try refining your search...</p>
                            </div>    
                        )
                    }
                    <div className="h-full w-full grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 3xl:grid-cols-4 gap-10 overflow-y-auto p-10">
                        {searchResults.tools && Object.keys(searchResults.tools).map((repo) => (
                            ...
                            <div className="w-full px-10">
                                ...
                                { category === "System Tools" && (
                                    <p className="text-sm p-4 bg-warning-50 rounded-xl mt-4 text-warning-950">
                                        <IoWarning className="inline text-lg mb-1 mr-1"/> These are lower level tools. Most interact directly with your workstation.
                                    </p>
                                )}
                                                    ...
                                                    <Button
                                                        ...
                                                        onPress={() => {
                                                            if (!priorityTools["From URL"].map((t) => t.url).includes(url)) {
                                                                ...
                                                                removeTool(url);
                                                                priorityTools["From URL"] = priorityTools["From URL"].filter((t) => t.url !== url)
                                                            }
                                                            setUrl("")
                                                        }}
                                                    />
