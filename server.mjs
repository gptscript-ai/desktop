...
app.prepare().then(() => {
    ...
    let landingPage = `http://${hostname}:${port}`
    if (runFile) {
        // `--file` was provided, change the landing page URL to run the given script
    ...
    open(landingPage).then(() => {
        ...
        console.error(`Failed to open landing page ${landingPage}: ${err.message}`)
    })
